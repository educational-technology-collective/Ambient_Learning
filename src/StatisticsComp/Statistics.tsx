...
import {BiTask, BiConfused, BiSkipNext, BiWinkSmile, BiCard} from 'react-icons/bi'
import {MdDoneOutline, MdOutlineClose} from 'react-icons/md'
import {FaPoop} from 'react-icons/fa'
import './Statistics.css'

const Statistics: React.FC<{total: number, duration: number}> = ({total, duration}) => {

  let correctNum, incorrectNum, skipNum, knowNum, dontKnowNum, oneMoreNum, poorCardNum; 
        ...
        <IonCard className='statistics-card'>
          <IonCardContent>
            <IonCardTitle className='card-title'>
              Cards Complete <BiTask size='1.2rem' className='card-icon'/> <span className='card-values'>{total}</span>
            </IonCardTitle>
          </IonCardContent>
        </IonCard>
